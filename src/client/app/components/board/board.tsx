...
export default class Board extends Component<IBoardProps, IBoardState> {
  get activeTileData(): IBoardTerritory {
    ...
    return this.props.boardData.territories.find(
      (t) => t.title === this.props.activeTerritory
    );
  }

  getLocation = (territoryName: string): { x: number; y: number } => {
    const territory = this.props.boardData.territories.find(
      (t) => t.title === territoryName
    );
  ...
  getColour = (territoryName: string): string => {
    ...
    const player = this.props.turnData.players.find((p) =>
      p.ownedTerritories.includes(territoryName)
    );
  ...
  alert_coords = (evt: MouseEvent) => {
    ...
    console.log(
      "(" + Math.round(cursorpt.x) + ", " + Math.round(cursorpt.y) + ")"
    );
  };
