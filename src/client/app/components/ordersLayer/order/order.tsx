        ...
        {props.moveType === "support" && (
          <Fragment>
            <path
              ...
              fill="transparent"
              stroke={markerColour}
              d={`M${this.calculateLocation(
                toLocation,
                fromLocation
              )} Q${supportFromLocation} ${this.calculateLocation(
                supportFromLocation,
                toLocation
              )}`}
            />
            <path
              ...
              d={`M${this.calculateLocation(
                toLocation,
                supportFromLocation
              )} L ${this.calculateLocation(supportFromLocation, toLocation)}`}
            />
          </Fragment>
        )}
        {props.moveType === "move" && (
          <path
            ...
            d={`M${this.calculateLocation(
              toLocation,
              fromLocation
            )} L${this.calculateLocation(fromLocation, toLocation)}`}
          />
        )}
      </Fragment>
    );
