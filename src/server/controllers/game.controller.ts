...
class GameController {
  ...
  static async join(req: Request, res: Response) {
    ...
    if (!(gameID && playerID))
      return res
        .status(400)
        .send("Require both gameID and playerID in request");
    const [err, game] = await to(
      GameService.joinGame(gameID, playerID, colour)
    );
    if (err) {
...
router.route("/").post(checkAuthentication, GameController.create);
router.route("/latest").get(checkAuthentication, GameController.viewLatest);
router.route("/:game_id").get(checkAuthentication, GameController.view);
router.route("/join").post(checkAuthentication, GameController.join);
router.route("/:game_id/start").post(checkAuthentication, GameController.start);
router.route("/:game_id/next").post(
