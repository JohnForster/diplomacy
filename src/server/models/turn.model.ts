...
turnSchema.methods.getMoves = function (playerID: any): IMove[] {
  const player: IPlayerStateDB = this.players.find((p: any) =>
    p.playerID.equals(playerID)
  );
  return player.moves;
};

turnSchema.methods.addMoves = function (
  ...
  if (
    ...
    console.error(
      moves.filter(
        (move) => !validateMove(toJSON(this), move, playerID.toString())
      )
    );
    return;
  }
...
turnSchema.methods.addPlayer = function (playerID: string, colour?: string) {
  ...
  if (
    colour &&
    this.players.find((player: IPlayerStateDB) => player.colour === colour)
  ) {
