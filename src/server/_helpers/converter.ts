...
const getUnits = (empire: Nation, apiState: IApiStateJSON) => {
  const unitsInApiFormat = Object.entries(apiState.Units).filter(
    ([, u]) => u.Nation === empire
  );
  const unitsInGameFormat: IUnit[] = unitsInApiFormat.map(
    ([locationId, u]) => ({
      unitType: u.Type,
...
const updateOwnedTerritories = (
  apiState: IApiStateJSON,
  players: IConversionData["players"]
): IConversionData["players"] => {
  const units = Object.entries(apiState.Units) as [
    ProvinceId,
    { Nation: Nation; Type: UnitType }
  ][];
  const convertedUnits = units.map(([id, unit]) => [
    getLocationFromId(id),
    unit,
  ]) as [string, { Nation: Nation; Type: UnitType }][];
  const noUnitAtTerritory = (territory: string) =>
    !convertedUnits.find(([location]) => location === territory);
...
export const apiToGameState = (
  ...
  const players: IConversionData["players"] =
    apiState.Type === "Adjustment"
...
const convertOrder = (move: IMove): ApiOrder => {
  ...
  switch (move.moveType) {
    case "build":
      return [OrderType.BUILD, move.buildType];
    case "convoy":
      return [OrderType.CONVOY, convoyFrom, to];
    case "disband":
      return [OrderType.DISBAND];
    case "hold":
      return [OrderType.HOLD];
    case "move":
      return [OrderType.MOVE, to];
    case "moveViaConvoy":
      return [OrderType.MOVEVIACONVOY, from, to]; // Not certain about this
    case "support":
      return [OrderType.SUPPORT, supportFrom, to];
  }
};
